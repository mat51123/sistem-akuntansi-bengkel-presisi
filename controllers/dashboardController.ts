...
export async function getDashboard(req: Request, res: Response) {
  try {
    ...
    `);
    const totalAset = Number(asetResult[0]?.total_debit || 0) - Number(asetResult[0]?.total_kredit || 0);
    ...
    `);
    const totalKewajiban = Number(kewajibanResult[0]?.total_kredit || 0) - Number(kewajibanResult[0]?.total_debit || 0);

    // 3. Jumlah Akun Aktif
    const akunResult = await db.query('SELECT COUNT(*) as count FROM akun');
    const jumlahAkun = Number(akunResult[0]?.count || 0);
    ...
    `);
    const totalPendapatan = Number(pendapatanResult[0]?.total || 0);
    ...
    `);
    const totalBeban = Number(bebanResult[0]?.total || 0);
    const labaBersih = totalPendapatan - totalBeban;

    // 5. Ambil 5 transaksi terbaru
    const recentTransactionsRaw = await db.query(`
    ...
    const recentTransactions = recentTransactionsRaw.map(tx => ({
      ...tx,
      total_nominal: Number(tx.total_nominal || 0)
    }));
